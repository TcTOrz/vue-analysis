...
function sameVnode (a, b) {
  return (
    // key 必须相同，另外 undefined === undefined => true
    a.key === b.key &&
 ...
 */
  function createComponent (vnode, insertedVnodeQueue, parentElm, refElm) {
    ...
    if (isDef(i)) {
      ...
      if (isDef(i = i.hook) && isDef(i = i.init)) {
        // init钩子函数执行 core/vdom/create-component中installComponentHooks注册
   ...
   * 更新节点
   *   全量的属性更新
   *   如果新老节点都有子节点，则递归执行 diff
   *   如果新节点有子节点，老节点没子节点，则新增子节点
   *   如果老节点有子节点，新节点没子节点，则删除子节点
   *   更新文本节点
   */
  function patchVnode (
    oldVnode,
  ...
  // oldVnode 旧VNode节点，可以是不存在或者是一个DOM对象
  // vnode 表示执行_render后返回的VNode节点
  // hydrating 表示是否是服务端渲染
  // removeOnly 给trainsition-group使用
  /**
   * vm.__patch__
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly) {
    ...
    if (isUndef(oldVnode)) {
      ...
      // <div id="app"><self-component></self-component></div>
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        // oldVnode.elm是真实DOM，emptyNodeAt方法传入
