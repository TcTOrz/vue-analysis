...
export function queueWatcher (watcher: Watcher) {
  const id = watcher.id
  if (has[id] == null) { // 保证同一个watcher只会添加一次
    has[id] = true
    if (!flushing) {
      // 未处于刷新队列状态，直接入队
      queue.push(watcher)
      ...
      // 倒序遍历
      // 如果flushing为true，那么当增加新的queue时，该watcher会立即执行。
      // 由于index是全局注册的，能够找出最快执行数组的位置。并且队列仍然有序。
      let i = queue.length - 1
      while (i > index && queue[i].id > watcher.id) {
        i--
      }
      queue.splice(i + 1, 0, watcher) // 最近位置插入当前Watcher
    }
    // queue the flush
    if (!waiting) {
      waiting = true
      ...
      if (process.env.NODE_ENV !== 'production' && !config.async) {
        // 直接刷新队列
        // 同步执行，Vue默认是异步执行，如果设置为同步，性能可能会变差
        flushSchedulerQueue()
        return
      }
