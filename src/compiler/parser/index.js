 ...
 */
  ...
  parseHTML(template, {
    ...
    start (tag, attrs, unary, start, end) {
      ...
      // 非生产环境，在 ast 对象上添加 一些 属性，比如 start、end
      if (process.env.NODE_ENV !== 'production') {
      ...
      if (!root) {
        ...
        if (process.env.NODE_ENV !== 'production') {
          // 检查根元素，对根元素有一些限制，
          // 比如：不能使用 slot 和 template 作为根元素，也不能在有状态组件的根元素上使用 v-for 指令
...
function processAttrs (el) {
  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) {
      ...
      if (bindRE.test(name)) { // v-bind // v-bind, <div :id="test"></div>
